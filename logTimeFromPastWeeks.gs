...
function trackerTestScript(day1, day2){
  ...
  var optionalArgs = {
    ...
    timeMax: (day2).toISOString(),
    showDeleted: false,
    singleEvents: true,
    maxResults: 1000,
    orderBy: 'startTime',
  };
  // Logger.log('%s - %s', day1, day2);
...
for (i=0; i<=(number Of Weeks); i++) {
  ...
  var endWeek = new Date(endDay.getTime() + week(i+1));
  
  list.push(startWeek, endWeek)
  
}
// If a cell in row 1 of 'Calendar Data by Week' equals (day1 - 1 day), then insert data into that column
